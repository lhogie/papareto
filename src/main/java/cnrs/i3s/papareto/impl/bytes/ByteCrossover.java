...
public class ByteCrossover<E> extends NewChildOperator<E, byte[]>
{
	...
	@Override
	public byte[] createNewChild(Population<E, byte[]> p, Random r)
	{
		byte[] i1 = p.getRepresentation().fromObject(p.binaryTournament(r).object);
		byte[] i2 = p.getRepresentation().fromObject(p.binaryTournament(r).object);
